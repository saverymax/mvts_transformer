# Generate the runs with the station variable
horizons="1 5 10"
chems="no2 pm10 pm25"
cd generated_experiments/finetune
rm run_all_finetune_with_station.sh
for h in $horizons
do
    for c in $chems
    do 
        RUN_DESC="bxl_finetune_lre4_station_h-${h}_${c}"
echo $RUN_DESC
...
python $VSC_DATA/thesis/mvts_transformer/src/main.py \\
    ...
    --comment \"no pretrained finetuning with station for ${RUN_DESC}\" \\
    --name ${RUN_DESC} \\
    ...
    --val_ratio=0.1" > run_${RUN_DESC}.PBS 
        echo "qsub run_${RUN_DESC}.PBS" >> run_all_finetune_with_station.sh
    done
done
